...
The Go backend reports most application errors as **HTTP 200** with a `{label, message}` body and no `data`, so axios does not reject and an absent `data` is the error signal. `getMyInstants` checks for it and rejects with the backend's message — the check is chained *after* the `.catch`, so its throw reaches the caller instead of being rewritten into the generic fallback. `getContent` and `playOnDiscord` still return/dereference `resp.data.data` without that check; neither blank-screens today, but both surface a worse message than the backend sent.
