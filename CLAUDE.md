...
The Go backend reports most application errors as **HTTP 200** with a `{label, message}` body and no `data`, so axios does not reject and an absent `data` is the error signal. Every unwrapping call goes through `unwrapData`, which rejects with an `ApiError(body.label, body.message)` when `data` is missing; the axios `.catch` alongside it rejects with `toApiError(err)`, reading `label`/`message` off the same envelope and falling back to a generic string only when the backend supplied none. Two ordering rules make this work: for `getMyInstants` the envelope check is chained *after* the `.catch` so its throw is not rewritten into the fallback, and for `getContent`/`playOnDiscord` it sits outside the `try` so it is not caught by the handler meant for transport errors. `markHealth(true)` fires as soon as a response arrives, before the envelope is judged — an error body still means the server answered. See **Internationalization** below for how `label` becomes display text.
